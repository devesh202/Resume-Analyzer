...
const Home = () => {
    const {generateReport,loading,reports, downloadResume } = useInterview();
    ...
    const handleGenerateResume = async () => {
        if (reports.length === 0) {
            toast.error("No reports available. Generate an interview report first.");
            return;
        }
        const latestReport = reports[0];
        const response = await downloadResume(latestReport._id);
        if(response.success){
        ...
        <div className="app-container">
            ...
            <aside className="sidebar">
                ...
                <nav className="sidebar-nav">
                    ...
                    <button className="nav-item" onClick={() => document.querySelector('.reports-section')?.scrollIntoView({ behavior: 'smooth' })}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.8" stroke="currentColor" width="18" height="18">
                                ...
                                <div key={report._id} onClick={()=>navigate(`/interview/result/${report._id}`)} className="report-card">
                                    ...
                                    <div className="report-card-actions">
                                        <button
                                            onClick={async (e) => {
                                                e.stopPropagation();
                                                const response = await downloadResume(report._id);
                                                if (response.success) {
                                                    toast.success("Resume downloaded successfully!");
                                                } else {
                                                    toast.error(response.message || "Failed to generate resume");
                                            ...
                                            disabled={loading}
                                            className="report-card-resume-btn"
